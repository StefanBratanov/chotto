...
public class SequencerClient {
  ...
  public Optional<BatchContribution> tryContribute(final String sessionId) {
    ...
    if (!contributionVerification.schemaCheck(response.body())) {
      LOG.error(
          "The received contribution does not match the defined contribution json schema. Exiting the ceremony!!!");
      System.exit(1);
    }
    ...
    if (!contributionVerification.subgroupChecks(batchContribution)) {
      LOG.error(
          "The received contribution does not pass the point checks. Exiting the ceremony!!!");
      System.exit(1);
    }
